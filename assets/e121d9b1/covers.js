$(function(){
                    ...
                    success: function(response){
                        ...
                        if(response.result === 'success'){
                            var html = $(photoTemplate
                                .replace(/\{\{photo_id\}\}/g, response.cover.id)
                                .replace(/\{\{photo_image\}\}/g, response.cover.image)
                                .replace(/\{\{photo_description\}\}/g, ''))
                                .hide();

                            var prevId = $('tr[data-id='+( response.cover.id - 1 )+']', photosBody);
                            if(prevId.get(0)){
                                prevId.before(html);
                            } else {
    ...
    photosBody.on('input propertychange', '.photo-description', function(){
        var saveBtn = $('#save_'+this.dataset.id);
    ...
    photosBody.on('change', '.change-image-input', function(){
        ...
        formData.append('Cover[image]', fileData);
        $.ajax({
            ...
            success: function(response){
                ...
                if(response.result === 'success'){
                    tr.find('.plugin-box').attr('href', response.cover.image).children('img');
                    notify.success(response.message);
                }else{
                    alert(response.error);
                }
