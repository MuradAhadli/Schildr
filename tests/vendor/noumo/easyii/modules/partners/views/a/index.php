...
$this->title = Yii::t('easyii', 'Partners');
...
<?php if($data->count > 0) : ?>
    ...
    <?php foreach($data->models as $item) : ?>
            <tr data-id="<?= $item->primaryKey ?>">
                ...
                <td><a href="<?= Url::to(['/admin/'.$module.'/a/edit/', 'id' => $item->primaryKey]) ?>"><?= $item->full_name ?></a></td>
                <td class="status">
                    <?= Html::checkbox('', $item->status == Partners::STATUS_ON, [
                        'class' => 'switch',
