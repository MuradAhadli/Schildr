...
$this->title = Yii::t('easyii', 'mailaddresses');
...
<?php if($data->count > 0) : ?>
    <table class="table table-hover">
        <thead>
            <tr>
                ...
                <th><?= Yii::t('easyii', 'Technical name')?></th>
                <th><?= Yii::t('easyii', 'Email')?></th>
                <th style="display: none" width="100"><?= Yii::t('easyii', 'Status') ?></th>
                <th width="120"></th>
            </tr>
        </thead>
        <tbody>
    <?php foreach($data->models as $item) : ?>
            <tr data-id="<?= $item->primaryKey ?>">
                ...
                <?php endif; ?>
                <td><a href="<?= Url::to(['/admin/'.$module.'/a/edit', 'id' => $item->primaryKey]) ?>"><?= StringHelper::truncate(strip_tags($item->name), 128) ?></a></td>
                <td><?= StringHelper::truncate(strip_tags($item->tech_name), 128) ?></td>
                <td><?= StringHelper::truncate(strip_tags($item->email), 128) ?></td>
                <td style="display: none" class="status vtop">
