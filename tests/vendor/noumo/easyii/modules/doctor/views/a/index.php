...
$this->title = Yii::t('easyii', 'Doctor');
    ...
    <?php foreach($data->models as $item) : ?>
            <tr data-id="<?= $item->primaryKey ?>">
                ...
                <td><a href="<?= Url::to(['/admin/'.$module.'/a/edit/', 'id' => $item->primaryKey]) ?>"><?= $item->name ?></a></td>
                    ...
                    <td><a href="<?= Url::to(['/admin/department/a/edit/', 'id' => $item->department_id]) ?>" target="_blank">
                        <?php
                            if(isset($item->department))
                                echo $item->department->name;
                        ?>
                    </a></td>

                <td class="status">
                    <?= Html::checkbox('', $item->status == Doctor::STATUS_ON, [
                        'class' => 'switch',
