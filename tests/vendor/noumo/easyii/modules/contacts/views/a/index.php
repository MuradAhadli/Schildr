...
$this->title = Yii::t('easyii', 'Contacts');
...
<?php if($data->count > 0) : ?>
    <table class="table table-hover">
        <thead>
            <tr>
                ...
                <th width="150"><?= Yii::t('easyii', 'Type') ?></th>
    ...
    <?php foreach($data->models as $item) : ?>
            <tr data-id="<?= $item->primaryKey ?>">
                ...
                <td><a href="<?= Url::to(['/admin/'.$module.'/a/edit/', 'id' => $item->primaryKey]) ?>"><?= Constants::getContactsTypes()[$item->type] ?></a></td>
                <td class="status">
                    <?= Html::checkbox('', $item->status == Contacts::STATUS_ON, [
                        'class' => 'switch',
